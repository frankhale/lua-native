...
**Cross-context values.** A table handle, environment, coroutine or Lua function belonging to a *different* context is rejected by `set_global`, `set_metatable`, `release`, `resume`, `create_coroutine` and `execute_script_in` alike — its registry index addresses an unrelated state. To copy data between contexts, read it by value: `b.set_global('cfg', a.get_global('cfg'))`, not `a.get_global_ref('cfg')`. Plain objects (including registered class instances and JS-created userdata) are deep-copied across as ordinary data; a table handle is a Proxy whose own keys are its API, so before CR-17 the same "deep copy" policy silently produced a table of method names instead of the data.

**Starting one requires that this thread is not already inside the Lua state.** Handing the `lua_State` to a worker thread is only safe if nobody else holds it, and a Lua state may be touched by one thread at a time. Both launchers therefore refuse, synchronously, on three of the four conditions that make `reset()` refuse: another async run in flight, Lua executing on this thread (a host callback, a metamethod, a `__gc` finalizer), or another lua-native call on the stack running your JavaScript (a type converter, a definition-object getter, a `Proxy` trap). (`reset()`'s fourth is its own re-entrancy, which does not apply here.) All three are declared as one shared policy, `lua_occupancy::kExclusive` — see `src/lua-native.h`. Before CR-15 only the first was checked, and a registered callback calling `execute_script_async()` put two threads in one `lua_State` — a deterministic segfault. Use `execute_async()` to start work from inside a callback: it is coroutine-driven and stays on the main thread, so it is not restricted this way.
