...
| For | Read |
|---|---|
| What is covered, what to run, what reopens review | `CORRECTNESS.md` §15 |
| Platform / CI scope (binding) | `CORRECTNESS.md` §14 |
| What the final action items turned out to be | `CORRECTNESS.md` §13 |
| Deferred items still cited from source comments | `CODE-REVIEW-LEDGER.md` |
| The individual passes | `CODE-REVIEW-1.md` … `CODE-REVIEW-22.md` |
| How to *run* the sanitizers (live) | `SANITIZERS.md` |
...
> **answered**: `CORRECTNESS.md` §15 is the "done" it was reaching
...
**Part II — The programme (CR-17 → CR-22).** Was `CORRECTNESS.md`
...
> scope** — see `CORRECTNESS.md` §14, which is binding. Do not act
> on them, and do not re-raise the question.

**Part III — The sanitizer assessment (July 2026).** Was
`SANITIZERS-THOUGHTS.md`, merged here on August 4, 2026. The judgment behind the
four sanitizer harnesses: what a sanitized build actually buys, where it is
load-bearing versus theatre, and the stress-test evidence.

> **Superseded on its forward-looking claim, and accurate on everything else.**
> Its conclusion — *"what remains is not more tooling but discipline"* — was
> overtaken almost immediately: six generated instruments were built after it
> (CR-18 through CR-22). It also names the exception-abort class as one the
> sanitizers cannot touch, covered only by a hand-written behavioural matrix;
> CR-18 replaced that with a 297-cell generated search that came back clean.
>
> Its core judgment stands and is still the reason `test-ts-asan` is the one to
> run before shipping: **a sanitizer answers "did anything go wrong on the paths
> I ran", not "is the code correct"**, so it is worth exactly as much as the
> adversarial paths you drive through it. The operational how-to was always in
> `SANITIZERS.md`, which remains live.

Section numbers in Part II are unchanged, so references of the form
"HISTORY §11" still resolve. Parts I and III have named rather than numbered
sections, so the three parts cannot collide.
...
`CODE-REVIEW-LEDGER.md` is triaged, accepted risk (M4/M5 remainders, H9c, M11,
...
*Split from `CORRECTNESS.md` §§1–12, August 4, 2026. Every action
...
# Part III — The sanitizer assessment (July 2026)

*Merged from `SANITIZERS-THOUGHTS.md`, August 4, 2026. The operational how-to
is in `SANITIZERS.md`, which is live; this is the judgment behind it. Superseded
on "what remains is not more tooling" — six instruments followed; see above.*
