...
// matrix enumerated. `tools/invariants/` freezes the counts these were
// derived from.
...
export const FRAMES = [
  // --- Lua is executing: the callback runs from a Lua C frame -------------
  {
    ...
    trigger: (lua) => lua.execute_script('return probe_hostile()'),
  },
  {
    id: 'metamethod_index',
    ...
    install: (lua, act) => {
      ...
      lua.set_metatable('probe_t', { __index: act });
    },
    trigger: (lua) => lua.execute_script('return probe_t.anything'),
  },
  {
    ...
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __newindex: act });
    },
    trigger: (lua) => lua.execute_script('probe_t.x = 1'),
  },
  {
    id: 'metamethod_add',
    ...
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __add: act });
    },
    trigger: (lua) => lua.execute_script('return probe_t + 1'),
  },
  {
    ...
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __tostring: act });
    },
    trigger: (lua) => lua.execute_script('return tostring(probe_t)'),
  },
  {
    id: 'metamethod_lt',
    describe: '__lt (comparison dispatch)',
    install: (lua, act) => {
      lua.execute_script('probe_t = {} probe_u = {}');
      lua.set_metatable('probe_t', { __lt: act });
      lua.execute_script('setmetatable(probe_u, getmetatable(probe_t))');
    },
    trigger: (lua) => lua.execute_script('return probe_t < probe_u'),
  },
  {
    id: 'metamethod_call',
    describe: '__call',
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __call: act });
    },
    trigger: (lua) => lua.execute_script('return probe_t()'),
  },
  {
    ...
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __concat: act });
    },
    trigger: (lua) => lua.execute_script('return probe_t .. "x"'),
  },
  {
    id: 'gc_finalizer',
    ...
    install: (lua, act) => {
      lua.execute_script('probe_t = {}');
      lua.set_metatable('probe_t', { __gc: act });
    },
    trigger: (lua) => {
      lua.execute_script('probe_t = nil');
      lua.gc('collect');
      lua.gc('collect');
    },
  },
  ...
  },
  {
    ...
    trigger: (lua) => lua.execute_script('do local x <close> = probe_t end'),
  },
  {
    id: 'debug_hook',
  ...
  },
  {
    ...
    trigger: (lua) => lua.execute_script('return require("probe_missing_module")'),
  },
  {
    id: 'coroutine_resume',
  ...
  },
  {
    ...
    install: (lua, act) => lua.set_global('probe_hostile', act),
    trigger: (lua) => {
      const co = lua.create_coroutine('return function() coroutine.yield(probe_hostile()) end');
      const seen = [];
  ...
  },
  {
    ...
    install: (lua, act) => lua.set_global('probe_hostile', act),
    trigger: (lua) => lua.execute_script('return load(probe_hostile)'),
  },
  {
    id: 'class_method',
    describe: 'a registered class method',
    install: (lua, act) => {
      lua.register_class('Probe', { construct: () => ({ tag: 'probe' }), methods: { m: act } });
    },
    trigger: (lua) => lua.execute_script('local c = Probe.new() return c:m()'),
  },
  {
    id: 'class_constructor',
    describe: 'a registered class constructor',
    install: (lua, act) => {
      lua.register_class('Probe', { construct: act, methods: { m: () => 1 } });
    },
    trigger: (lua) => lua.execute_script('local c = Probe.new() return 1'),
  },
  {
    id: 'class_metamethod',
    describe: 'a registered class metamethod (__tostring)',
    install: (lua, act) => {
      lua.register_class('Probe', { construct: () => ({ tag: 'probe' }), metamethods: { __tostring: act } });
    },
    trigger: (lua) => lua.execute_script('local c = Probe.new() return tostring(c)'),
  },
  {
    id: 'userdata_method',
    describe: 'a userdata method',
    install: (lua, act) => lua.set_userdata('probe_ud', { tag: 'probe' }, { methods: { m: act } }),
    trigger: (lua) => lua.execute_script('return probe_ud:m()'),
  },
  {
  ...
  // --- Binding call, no Lua running: converters and marshalling ----------
  {
    ...
    install: (lua, act) => {
      lua.register_type_converter((v) => typeof v === 'object' && v !== null && 'probe' in v, act);
    },
    trigger: (lua) => lua.set_global('probe_v', { probe: true }),
  },
  {
