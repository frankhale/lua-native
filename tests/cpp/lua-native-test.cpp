...
TEST(LuaRuntimeCore, RegisterFunctionAndCall) {
  ...
  rt.RegisterFunction("adder", [](const std::vector<LuaPtr>& args) -> LuaPtr {
    ...
    return std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{a + b}});
  });
...
TEST(LuaRuntimeCore, SetGlobalAndGetGlobal) {
  const LuaRuntime rt;
  rt.SetGlobal("x", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{42}}));
...
TEST(LuaRuntimeCore, DeepRecursionCap) {
  ...
  // Walk down 100 levels; after that we expect monostate due to depth cap
  ...
  // At depth 100, the next child value should be monostate (cap reached at depth > 100)
  ASSERT_TRUE(std::holds_alternative<LuaTable>((*current)->value));
  const auto& tbl100 = std::get<LuaTable>((*current)->value);
  const auto it100 = tbl100.find("child");
  ASSERT_NE(it100, tbl100.end());
  ASSERT_TRUE(std::holds_alternative<std::monostate>(it100->second->value));
}
...
TEST(LuaRuntimeCore, HostFunctionReturnsArrayAndTable) {
  ...
  rt.RegisterFunction("mkArray", [](const std::vector<LuaPtr>&) -> LuaPtr {
    auto a = LuaArray{};
    a.push_back(std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{10}}));
    a.push_back(std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{20}}));
    return std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::move(a)}});
  });
  rt.RegisterFunction("mkTable", [](const std::vector<LuaPtr>&) -> LuaPtr {
    LuaTable t;
    t.emplace("k", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::string("v")}}));
    return std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::move(t)}});
  });
...
TEST(LuaRuntimeCore, SetGlobalComplexStructures) {
  ...
  {
    LuaArray arr;
    arr.push_back(std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{5}}));
    arr.push_back(std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{6}}));
    rt.SetGlobal("t", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::move(arr)}}));
  ...
  {
    LuaTable inner;
    inner.emplace("c", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{8}}));
    LuaTable outer;
    outer.emplace("a", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{7}}));
    outer.emplace("b", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::move(inner)}}));
    rt.SetGlobal("m", std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{std::move(outer)}}));
...
TEST(LuaRuntimeCore, FunctionReregistrationUsesLatest) {
  ...
  rt.RegisterFunction("f", [](const std::vector<LuaPtr>&) -> LuaPtr {
    return std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{1}});
  });
  rt.RegisterFunction("f", [](const std::vector<LuaPtr>&) -> LuaPtr {
    return std::make_shared<LuaValue>(LuaValue{LuaValue::Variant{2}});
  });
